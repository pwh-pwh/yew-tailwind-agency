...
#[function_component]
pub fn Solutions() -> Html {
    html! {
        <div>
            <SectionHeading title={"解决方案"} sub_title={"方案解决方案解决方案解决方案解决方案解决方案解决方案解决方案"}/>
            <div class="grid lg:grid-cols-2 mt-20 gap-20">
                <div>
                    <h3 class="mt-7 text-[32px] font-fold">{"强有力的工具"}</h3>
            ...
            <ul class="grid gap-3">
        {
            [1,2,3].map(|v| {
                html! {
                    <li
                class={classes!("p-5", "justify-between","rounded", "border", "border-zinc-100", if v == 1 {"bg-zinc-100"} else {""})}
                key={v}
              >
                <p class="text-lg font-bold">{"构建简单的生态系统"}</p>
                ...
                <svg
                    ...
                    class="shrink-0 ml-8 relative -top-2"
                  >
        ...
        <img src={"assets/pexels-photo-8348457.jpg"} alt="" />
            </div>
        </div>
    }
}
